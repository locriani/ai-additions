---
name: defect-density
description: Use when the user asks how often they introduce bugs — rate, density, escaped-to-production count, or where the bugs come from — because the answer needs defects correlated against churn over a window, not a git summary. Do NOT use for output rate (dev-velocity) or hours worked (time-estimation).
---
...
Use `${CLAUDE_PLUGIN_ROOT}/skills/defect-density/scripts/bug-tally.py` to automate the git parsing. Pass its JSON output into your analysis.
...
|---|---|---|
| "I'll just grep for fix: commits" | `A ⊊ B` | Misses hotfixes, reverts, bundled review findings, test-driven fixes, and plain-english descriptions. Under-counts by 2–5×. |
| "Low bugs/hour means I write good code" | `¬obs(P) ↛ ¬P` | Maybe. Or you have light review coverage and bugs are escaping. The escape rate matters more than the raw count. |
| "My DRE is 100% because nothing escaped" | `¬obs(P) ↛ ¬P` | Only knowable if you have explicit production monitoring. Absence of evidence is not evidence of absence. |
| "Jones's numbers don't apply to me — I use AI" | `own_data ≻ benchmark` | The research predates AI-assisted dev. Your escape funnel data is more trustworthy than the benchmarks; use them as context, not gospel. |
| "The bugs/KLOC seems too low, maybe I miscounted" | `felt(P) ↛ tested(P)` | More likely: you counted only fix: commits. Re-run with the full source taxonomy. |
