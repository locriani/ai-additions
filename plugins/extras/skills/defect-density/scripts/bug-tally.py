...
def get_commits(repo: str, since: str | None) -> list[dict]:
    ...
    # `sep` / `rec` are what we split git's OUTPUT on (real NUL bytes are valid
    # inside a Python str). `fmt_sep` / `fmt_rec` are what go in the
    # --pretty=format ARGUMENT: git's own %x00 escape, so the argv element holds
    # the ASCII chars "%x00" and never a raw NUL. A raw NUL in argv is rejected
    # by execve ("ValueError: embedded null byte"); git expands %x00 to a real
    # NUL only in its stdout, which subprocess text mode decodes without issue.
    # The record terminator is NUL-wrapped too (not a plain "END_BODY" token):
    # git commit messages cannot contain a NUL byte, so a commit body that
    # literally contains the text "END_BODY" can no longer split a record.
    sep = "\x00COMMIT\x00"
    rec = "\x00END_BODY\x00"
    fmt_sep = "%x00COMMIT%x00"
    fmt_rec = "%x00END_BODY%x00"
    cmd = ["log", f"--pretty=format:%H{fmt_sep}%s{fmt_sep}%b{fmt_rec}"]
    if since:
        cmd += [f"--since={since}"]
