...
description: Use when the user asks how long they have spent on something — hours since a date, build, or commit — because working time has to be inferred from commit clustering, not read off a log. Do NOT use for output rate (dev-velocity) or a plain git-log recap.
---
...
Use the `Agent` tool with `subagent_type: "general-purpose"`. The subagent does NOT have access to this skill (skills are not auto-loaded into subagents). You must inline the instructions.
...
Two scripts ship with this skill at `${CLAUDE_PLUGIN_ROOT}/skills/time-estimation/scripts/`:
