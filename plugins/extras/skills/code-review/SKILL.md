---
name: code-review
description: Use when the user asks for a review of code that exists — a PR, a diff, a branch, a worktree, "review what I just did" — because a review needs orthogonal axes dispatched in parallel and tests-that-lie triaged first, which inline reading skips. Do NOT use for a one-line typo/comment/config change (review it inline) or for design feedback on code not yet written.
---
...
|---|---|---|
| "It's small enough — I'll just read through it." | `felt(P) ↛ tested(P)` | "Small" is a feeling; the escape hatch is a test. Run it, or dispatch. |
| "I already reviewed as I wrote it." | `author ⊨ intent, ⊭ behavior` | You reviewed what you meant. A reviewer reads what you wrote. Different artifacts. |
| "Tests pass, that's enough." | `↑proxy ↛ ↑target` | Green proves the tests agree with the code, not that either is right. Tests-that-lie come first for a reason. |
| "The user is in a rush." | `urgency ↛ ↓risk` | Rush changes what you report, never whether you looked. |
| "I'll review at PR time, this is just a WIP commit." | `planned ↛ landed` | A review you haven't run is not scheduled, it's skipped. |
| "Nothing here is architecturally significant." | `¬obs(P) ↛ ¬P` | Deciding nothing is significant requires the review you're skipping. |
| "The diff is medium — single reviewer is fine." | `A ⊊ B` | One reviewer finds one axis of defect. The dispatch pattern exists because the axes are independent. |
