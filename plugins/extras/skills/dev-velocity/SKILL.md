---
name: dev-velocity
description: Use when the user asks how much code they produce per unit time — lines per hour, per commit, throughput, "how productive am I" — because the answer needs churn measured over a window, not a commit list. Do NOT use for hours worked (time-estimation) or bug rates (defect-density).
---
...
Use the helper script at `${CLAUDE_PLUGIN_ROOT}/skills/dev-velocity/scripts/loc-stats.py` (see below). If not present, run manually:
...
|---|---|---|
| "Higher LOC/hr = more productive" | `↑proxy ↛ ↑target` | LOC measures output volume, not value. Deleting 1,000 lines of dead code is high-value, zero LOC/hr. |
| "The senior dev baseline is 100–150 LOC/hr" | `∄primary_source` — premise = ⊥ | No primary source. Brooks says 10 lines/day (shipped, debugged). McConnell says 2–16/hr for non-trivial work. |
| "I'll skip excluding generated files — it's faster" | `¬exclude(generated) ⊢ meaningless` | Generated files can add 10–100× artificial LOC. The number becomes meaningless without the exclusion. |
| "LOC/commit is too high — the project is unhealthy" | `↑proxy ↛ ↑target` | Large LOC/commit often just means squash workflow, not low commit discipline. Check before concluding. |
